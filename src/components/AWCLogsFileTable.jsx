...
const AWCLogsFileTable = () => {
  ...
  const [subfolders, setSubfolders] = useState({});
  const [expandedSubfolder, setExpandedSubfolder] = useState(null);
  const [selectedFileData, setSelectedFileData] = useState({}); // {subCode: data}

  const [currentPage, setCurrentPage] = useState(1);
  const foldersPerPage = 10;
  const [subfolderPages, setSubfolderPages] = useState({});
  ...
  const handleSubfolderToggle = (code) => {
    setExpandedSubfolder((prev) => (prev === code ? null : code));
    setSelectedFileData({}); // Reset file data on toggle
  };

  const handleFileClick = async (folderCode, subfolderCode, fileName) => {
    try {
      const fileKey = `${folderCode}/${subfolderCode}/${fileName}`;
      const response = await axios.get(`http://localhost:8000/api/v1/anganwadi/report?fileKey=${fileKey}`);
      ...
      setSelectedFileData({ [subfolderCode]: data });
    } catch (err) {
                        ...
                        <React.Fragment key={sub.code}>
                          ...
                          {expandedSubfolder === sub.code && (
                            <tr className="bg-purple-50 border-b">
                              <td colSpan={4} className="px-12 py-2 text-sm text-gray-800">
                                <strong>Sample Files:</strong>
                                <table className="mt-2 w-full text-sm bg-white border border-gray-200">
                                  <thead>
                                  ...
                                  <tbody>
                                    {sub.sampleFiles?.map((file, index) => (
                                      <tr
                                        key={index}
                                        className="hover:bg-gray-50 cursor-pointer"
                                        onClick={() =>
                                          handleFileClick(folder.code, sub.code, file)
                                        }
                                      >
                                ...
                                {selectedFileData[sub.code]?.length > 0 && (
                                  <div className="mt-4 overflow-auto border rounded">
                                    <table className="min-w-full text-sm text-left border border-collapse">
                                      ...
                                      <tbody>
                                        {selectedFileData[sub.code].map((row, idx) => (
